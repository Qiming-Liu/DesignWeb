function DragImgUpload(id, src, options) {
    this.me = $(id);
    var defaultOpt = {
        boxWidth: '200px',
        boxHeight: 'auto'
    }
    this.preview = $('<img src="' + src + '" class="mw-100 w-100 rounded" alt="image" style="cursor:pointer">');
...
DragImgUpload.prototype = {
    ...
    onDrop: function (e) {
        ...
        //检测文件是不是图片
        if (fileList[0].name.substr(-3, 3) != "jpg") {
            swal({
                title: "类型错误",
                text: "只接受JPG格式的文件！",
                icon: "error",
...
function imgUpload(src, img, url) {
